  ...
  return (
      <div className={`w-full  z-40 flex px-20 justify-between duration-150  py-4  top-0 left-0 ${scrolled ?'sticky  shadow':''} bg-white rounded-b-3xl `}>
        <Link to={"/"} className='flex space-x-2 place-items-center group duration-150 cursor-pointer'>
          <img src={require('../assets/media/logoo.png')} alt='logo' className='w-10 h-10  duration-150 rounded group-hover:bg-black group-hover:opacity-50 ' />
          ...
          <div>
            {user ? <UserDropd user={user} logout={logout}/> : !token && location.pathname != '/login' && location.pathname != '/register' && <Link to={'/login'} className=' p-2 rounded bg-[#51989b]  backdrop-blur-sm text-white hover:scale-105 duration-150 po flex'>
